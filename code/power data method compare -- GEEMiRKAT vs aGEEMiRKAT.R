...
for (scenario in 1:4) {
  out1 <- read.csv("E:/GEEMiRKAT/typeIerror/aGEEMiRKAT.typeIerror.csv", row.names = 1)
  out.typeIerror <- out1[, 1]

  position <- paste("E:/GEEMiRKAT/power/scenario", scenario, sep = "")
  setwd(position)
  out2 <- read.csv("aGEEMiRKAT.power.n=20.csv", row.names = 1)
  out.power <- out2
...
for (scenario in 1:4) {
  ...
  position <- paste("E:/GEEMiRKAT/power/scenario", scenario, sep = "")
  setwd(position)
  out2 <- read.csv("aGEEMiRKAT.power.n=40.csv", row.names = 1)
  out.power <- out2
  out <- t(cbind(out.typeIerror, out.power))
  rownames(out) <- seq(0, 2, 0.25)
...
filename.methods.20 <- "E:/GEEMiRKAT/plot/data/aGEEMiRKAT.comparison.20.rds"
filename.methods.40 <- "E:/GEEMiRKAT/plot/data/aGEEMiRKAT.comparison.40.rds"
